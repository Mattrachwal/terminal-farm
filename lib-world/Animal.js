class Animal {
    constructor(
        ...
        this.body = {
            ascii: ascii,
            bg_color: this.setFgColor(),
            fg_color: this.setFgColor(),
        };
        this.focus = {
            x_pos: this.x_pos,
            y_pos: this.y_pos,
    ...
    die(state, index) {
        const animals = state.resources[this.type].entities;
        state.resources[this.type].instances = animals.splice(index,1);
    }
    ...
    findFocus (state) {
		...
		if (this.hunger >= this.hunger_threshold) {
			this.findFood(state);
		} else if (!this.sex && this.sex_drive >= this.sex_drive_threshold) {
			this.findMate(state);
		} else {
    ...
    mate (state) {
        state.resources[this.type].entities.push(new Animal('baby','Ÿ', 238, 15, this.type, this.x_pos, this.y_pos));
    }
    ...
    searchPosition(state) {
        ...
        if (!this.sex && this.sex_drive > this.sex_drive_threshold) {
            const animals = state.resources[this.type].entities;
            animals.forEach( (animal) => {
                if (animal.sex && this.x_pos === animal.x_pos && this.y_pos === animal.y_pos) {
                    this.mate(state);
                    somethingFound = true;
                    animal.sex_drive = 0;
                    this.sex_drive = 0;
